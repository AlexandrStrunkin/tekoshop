...
$countCMP=count($compare_items);

// update basket counters
CModule::IncludeModule('aspro.optimus');
$arCounters = COptimus::updateBasketCounters(array('READY' => array('COUNT' => $normalCount, 'TITLE' => $arResult['allSum_FORMATED'], 'HREF' => $arParams["PATH_TO_BASKET"]), 'DELAY' => array('COUNT' => $delayCount, 'TITLE' => $arResult["DELAY_PRICE"]["SUMM_FORMATED"], 'HREF' => $arParams["PATH_TO_BASKET"]), 'COMPARE' => array('COUNT' => $countCMP, 'HREF' => $arParams["PATH_TO_COMPARE"]), 'PERSONAL' => array('HREF' => $arParams["PATH_TO_AUTH"])));
