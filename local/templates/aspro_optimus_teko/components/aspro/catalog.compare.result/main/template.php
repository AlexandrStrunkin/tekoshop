						...
						<?foreach($arResult["ITEMS"] as &$arElement){?>
							<td>
								<div class="item_block">
									<span onclick="CatalogCompareObj.MakeAjaxAction('/catalog/compare.php?action=DELETE_FROM_COMPARE_RESULT&ID=<?=$arElement['ID']?>', 'Y');" class="remove" title="<?=GetMessage("CATALOG_REMOVE_PRODUCT")?>"><i></i></span>
						...
						foreach ($arResult["SHOW_FIELDS"] as $code => $arProp){
							if(!in_array($code, $arUnvisible)){
								...
								foreach($arResult["ITEMS"] as &$arElement){
									...
									$arCompare[] = trim($arPropertyValue);
								}
								unset($arElement);
								if ($arResult['DIFFERENT'])
									$showRow = (count(array_unique($arCompare)) > 1);
								elseif (count(array_unique($arCompare)) == 1 && trim($arCompare[0])=='')
									$showRow = false;
								if ($showRow){?>
									<tr>
										<td>
