		...
		<?foreach($arResult["ITEMS"] as $arItem):?>
			...
			<?if($arItem["DETAIL_PICTURE"]["SRC"] || $arItem["PREVIEW_PICTURE"]["SRC"]):?>
				<div class="item s_<?=$i;?> <?=($isUrl ? "hover" : "");?> <?=($arItem["PROPERTIES"]["BANNER_SIZE"]["VALUE_XML_ID"] ? $arItem["PROPERTIES"]["BANNER_SIZE"]["VALUE_XML_ID"] : "normal");?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					...
					<?if($isUrl){?>
						<a href="<?=$arItem["PROPERTIES"]["URL_STRING"]["VALUE"]?>" class="opacity_block1 dark_block_animate" title="<?=$arItem["FORMAT_NAME"];?>" target="<?=$arItem["PROPERTIES"]["TARGETS"]["VALUE_XML_ID"]?>"></a>
					<?}
								...
								<?if($arItem["PREVIEW_TEXT"]){?>
									<div class="wr_block price">
										<span class="wrap_outer_desc">
											<?if($isUrl){?>
												<a class="outer_text_desc" href="<?=$arItem["PROPERTIES"]["URL_STRING"]["VALUE"]?>" target="<?=$arItem["PROPERTIES"]["TARGETS"]["VALUE_XML_ID"]?>">
											<?}else{?>
