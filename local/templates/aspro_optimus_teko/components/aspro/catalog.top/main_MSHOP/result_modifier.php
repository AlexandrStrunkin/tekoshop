...
if (!empty($arResult['ITEMS'])){
	...
	array_unique($itemsIds);
	
	$ob = CIBlockElement::GetList(array(), array('ID' => $itemsIds), false, false, array('ID', 'IBLOCK_ID') );
	$hitProps = array();
	while ($el = $ob->GetNextElement()){
		$prop = $el->GetProperty('HIT');
		$fields = $el->GetFields();
		
		$hitProps[$fields['ID']] = $prop;
	}

	foreach ($arResult['ITEMS'] as $key => $arItem) {
		$arResult['ITEMS'][$key]['PROPERTIES']['HIT'] = $hitProps[$arItem['ID']];
	}
}?>